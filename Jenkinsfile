...
stage('Kubernetes Deploy') {
  when {
    expression {
      // Only run this stage for dev, uat, release/*, or hotfix/*
  ...
  steps {
    script {
      ...
      // Determine namespace based on branch prefix
      def namespace = [
        'dev': 'dev',
        'uat': 'uat',
        'release': 'prod',
      ...
      // Select kubeconfig and manifest path per branch
      switch (branch) {
        case 'dev':
      ...
      // Prepare image tag with build number
      ...
      withCredentials([
        ...
        sh """
          # Use the injected kubeconfig file
          export KUBECONFIG=$KCFG

          # Fail if manifest file is missing
          if [ ! -f ${deploymentFile} ]; then
            echo "❌ File ${deploymentFile} not found!"
            exit 1
          fi
        ...
        """
        // Use '#' for comments inside the shell block
      }
    }
  }
}
